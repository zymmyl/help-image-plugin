...
const PLUGIN_NAME = 'help-image-plugin'
...
const getTime = () => {
  let cm = `git -C ./plugins/${PLUGIN_NAME}/ log -1 --oneline --pretty=format:"%cd" --date=format:"%m-%d %H:%M"`
  try {
    return execCommandSync(cm)?.trim()
  } catch (error) {
...
const getLogLines = () => {
  let cm = `git -C ./plugins/${PLUGIN_NAME}/ log -20 --oneline --pretty=format:"%h||[%cd]  %s" --date=format:"%m-%d %H:%M"`
...
const replyGitError = async (reply, err, stdout) => {
  ...
  if (/Failed to connect|unable to access|Connection reset/g.test(errMsg)) {
  ...
  if (errMsg.includes('be overwritten by merge')) {
    await reply(
      msg +
      `存在冲突：\n${errMsg}\n` +
      '请解决冲突后再更新，或者发送 #更新帮助图强制更新，放弃本地修改'
    )
    return
  }
...
export { checkGit, getCommitId, getTime, getLogLines, replyGitError, PLUGIN_NAME }
